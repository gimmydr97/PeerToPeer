...
contract Mayor {
    ...
    function cast_envelope(bytes32 _envelope) canVote public {
        ...
        if(envelopes[msg.sender] == 0x0) // => NEW, update on 17/05/2021, serve a vedere se nn è già presente in envelopes
    ...
    function open_envelope(uint _sigil, bool _doblon) canOpen public payable {
     ...
     /// @notice update yaySoul and naySoul with the souls in the envelope
        if(_doblon == true){
     ...
     /// @notice update the souls structure with the voter data
        souls[msg.sender] = Refund({soul:msg.value, doblon:_doblon});
    ...
    function mayor_or_sayonara() canCheckOutcome public {
        ...
        /// @notice refunding the losing voters and towards the souls destined for the mayor on the candidate address
        ...
        /// @notice refunding the losing voters and towards the souls destined for the escrow account on the escrow address
        else{
            for(uint i=0; i < voters.length; i++ ){
